...
mod tests {
    ...
    fn layout_relocates_all_data_rvas_together() {
        ...
        let original_get_std_handle_iat = layout.get_std_handle_iat;

        layout.relocate_data_sections(0x3000);

        assert_eq!(layout.rdata_rva, 0x3000);
        assert_eq!(layout.idata_rva, original_idata + 0x1000);
        assert_eq!(layout.written_rva, original_written + 0x1000);
        assert_eq!(layout.import_descriptor_rva, original_imports + 0x1000);
        assert_eq!(
            layout.get_std_handle_iat,
            original_get_std_handle_iat + 0x1000
        );
    }
}
...
fn emit_compiled_pe64_console(program: &IrProgram) -> Option<Vec<u8>> {
    ...
    if text_end > layout.rdata_rva {
        layout.relocate_data_sections(align_to(text_end, SECTION_ALIGNMENT));
        text = build_compiled_text(&layout, &image)?;
    }
...
impl Layout {
    fn new(
        ...
        let file_import_count = if has_file_read {
            4
        } else if has_file_ops || has_file_truncate {
            2
        } else {
            0
        };
        ...
        let create_file_iat = if has_file_read || has_file_ops || has_file_truncate {
            let value = next_iat;
            next_iat += 8;
            value
        } else {
        ...
        let close_handle_iat = if has_file_read || has_file_ops || has_file_truncate {
            let value = next_iat;
            next_iat += 8;
            value
        } else {
        ...
        let set_end_of_file_iat = if has_file_truncate {
            let value = next_iat;
            next_iat += 8;
            value
        } else {
    ...
    fn relocate_data_sections(&mut self, rdata_rva: u32) {
        let delta = rdata_rva - self.rdata_rva;
        self.rdata_rva = rdata_rva;
        self.idata_rva += delta;
        self.message_rva += delta;
        self.written_rva += delta;
        self.import_descriptor_rva += delta;
...
fn build_compiled_text(layout: &Layout, image: &ObjectImage) -> Option<Vec<u8>> {
    ...
    let needs_array_remove = image
        .relocations
        .iter()
        .any(|relocation| relocation.symbol == "array_remove");
    let needs_array_insert = image
        .relocations
        .iter()
        .any(|relocation| relocation.symbol == "array_insert");
    let needs_array_extend = image
        .relocations
        .iter()
        .any(|relocation| relocation.symbol == "array_extend");
    let needs_array_copy = image
        .relocations
        .iter()
    ...
    if needs_array_remove {
        helpers.array_remove = Some(layout.text_rva + code.len() as u32);
        emit_array_remove_helper(&mut code);
    }
    if needs_array_insert {
        helpers.array_insert = Some(layout.text_rva + code.len() as u32);
        emit_array_insert_helper(&mut code);
    }
    if needs_array_extend {
        helpers.array_extend = Some(layout.text_rva + code.len() as u32);
        emit_array_extend_helper(&mut code);
    }
    if needs_array_copy {
    ...
    if needs_path_stem {
        let string_slice = helpers.string_slice?;
        helpers.path_stem = Some(layout.text_rva + code.len() as u32);
        emit_path_stem_helper(&mut code, layout, string_slice);
    }
    if needs_path_without_extension {
        let string_slice = helpers.string_slice?;
...
fn compiled_symbol_rva(
    ...
    if symbol == "array_remove" {
        return helpers.array_remove;
    }
    if symbol == "array_insert" {
        return helpers.array_insert;
    }
    if symbol == "array_extend" {
        return helpers.array_extend;
    }
...
fn emit_string_concat_helper(code: &mut Vec<u8>, layout: &Layout, string_len_rva: u32) {
    ...
    code.extend_from_slice(&[0x48, 0x83, 0xc2, 0x08, 0x48, 0x89, 0x54, 0x24, 0x50]);
    ...
    code.extend_from_slice(&[0x48, 0x89, 0xc2, 0x49, 0x89, 0xc1]);
    code.extend_from_slice(&[
        0x4c, 0x8b, 0x44, 0x24, 0x30, 0x4c, 0x89, 0x02, 0x48, 0x83, 0xc2, 0x08, 0x49, 0x83, 0xc1,
        0x08,
    ]);
...
fn emit_array_fill_helper(code: &mut Vec<u8>) {
    code.extend_from_slice(&[
        0x48, 0x85, 0xc9, 0x74, 0x32, 0x48, 0x83, 0xec, 0x38, 0x48, 0x89, 0x54, 0x24, 0x28, 0x49,
        0x89, 0xca, 0x48, 0x8b, 0x41, 0x08, 0x4c, 0x8b, 0x59, 0x18, 0x48, 0x8b, 0x39,
    ]);
    code.extend_from_slice(&[
        0x48, 0x85, 0xc0, 0x74, 0x0f, 0x4c, 0x89, 0xd9, 0x48, 0x8b, 0x74, 0x24, 0x28, 0xf3, 0xa4,
        0x48, 0xff, 0xc8, 0xeb, 0xec, 0x31, 0xc0, 0x48, 0x83, 0xc4, 0x38, 0xc3, 0xb8, 0x01, 0x00,
        0x00, 0x00, 0xc3,
    ]);
}
...
fn emit_array_index_helper(code: &mut Vec<u8>, last: bool, count: bool, contains: bool) {
    code.extend_from_slice(&[0x48, 0x85, 0xc9]);
    let null_array = emit_short_jump_placeholder(code, 0x74);
    code.extend_from_slice(&[0x48, 0x85, 0xd2]);
    let null_value = emit_short_jump_placeholder(code, 0x74);
    code.extend_from_slice(&[
        0x57, 0x56, 0xfc, 0x49, 0x89, 0xd0, 0x4c, 0x8b, 0x51, 0x08, 0x4c, 0x8b, 0x59, 0x18, 0x4c,
        0x8b, 0x09, 0x31, 0xd2,
    ]);
    if last {
    ...
    let done = emit_short_jump_placeholder(code, 0x74);
    code.extend_from_slice(&[
        0x4c, 0x89, 0xcf, 0x4c, 0x89, 0xc6, 0x4c, 0x89, 0xd9, 0xf3, 0xa6,
    ]);
    let mismatch = emit_short_jump_placeholder(code, 0x75);
    if contains {
        code.extend_from_slice(&[0xb8, 0x01, 0x00, 0x00, 0x00, 0x5e, 0x5f, 0xc3]);
    } else if count {
        code.extend_from_slice(&[0x48, 0xff, 0xc0]);
    } else if last {
        code.extend_from_slice(&[0x48, 0x89, 0xd0]);
    } else {
        code.extend_from_slice(&[0x48, 0x89, 0xd0, 0x5e, 0x5f, 0xc3]);
    }
    if count || last {
    ...
    code.extend_from_slice(&[0x5e, 0x5f]);
    if contains {
        code.extend_from_slice(&[0x31, 0xc0]);
    }
    code.push(0xc3);
...
fn emit_array_set_helper(code: &mut Vec<u8>) {
    ...
    let failed = emit_short_jump_placeholder(code, 0x73);
    code.extend_from_slice(&[
        0x4c, 0x8b, 0x49, 0x18, 0x4d, 0x89, 0xca, 0x4c, 0x0f, 0xaf, 0xca, 0x48, 0x8b, 0x01, 0x4c,
        0x01, 0xc8, 0x4c, 0x89, 0xd1, 0x48, 0x89, 0xc7, 0x4c, 0x89, 0xc6, 0xf3, 0xa4, 0x31, 0xc0,
        0xc3,
    ]);
    let failure = code.len();
    code.extend_from_slice(&[0xb8, 0x01, 0x00, 0x00, 0x00, 0xc3]);
...
fn emit_array_push_helper(code: &mut Vec<u8>) {
    ...
    code.extend_from_slice(&[
        0x4c, 0x8b, 0x41, 0x18, 0x4d, 0x89, 0xc1, 0x4c, 0x0f, 0xaf, 0xc0, 0x4c, 0x8b, 0x11, 0x4d,
        0x01, 0xd0, 0x48, 0xff, 0x41, 0x08, 0x4c, 0x89, 0xc9, 0x4c, 0x89, 0xc7, 0x48, 0x89, 0xd6,
        0xf3, 0xa4, 0x31, 0xc0, 0xc3,
    ]);
    let failure = code.len();
    code.extend_from_slice(&[0xb8, 0x01, 0x00, 0x00, 0x00, 0xc3]);
    patch_short_jump(code, failed, failure);
...
fn emit_array_swap_helper(code: &mut Vec<u8>) {
    ...
    let done = emit_near_jump_placeholder(code, 0x84);
    code.extend_from_slice(&[
        0x45, 0x8a, 0x03, 0x8a, 0x08, 0x41, 0x88, 0x0b, 0x44, 0x88, 0x00, 0x49, 0xff, 0xc3, 0x48,
        0xff, 0xc0, 0x49, 0xff, 0xc9,
    ]);
    emit_short_jump_back(code, loop_start);
    let success = code.len();
    code.extend_from_slice(&[0x31, 0xc0, 0xc3]);
...
fn emit_array_swap_remove_helper(code: &mut Vec<u8>) {
    code.extend_from_slice(&[
        0x48, 0x85, 0xc9, 0x74, 0x49, 0x48, 0x8b, 0x41, 0x08, 0x48, 0x39, 0xc2, 0x73, 0x40, 0x48,
        0xff, 0xc8, 0x48, 0x89, 0x41, 0x08, 0x48, 0x39, 0xc2, 0x74, 0x31, 0x4c, 0x8b, 0x51, 0x18,
        0x4c, 0x8b, 0x19, 0x49, 0x89, 0xd0, 0x4d, 0x0f, 0xaf, 0xc2, 0x4d, 0x01, 0xd8, 0x49, 0x89,
        0xc1, 0x4d, 0x0f, 0xaf, 0xca, 0x4d, 0x01, 0xd9, 0x4d, 0x85, 0xd2, 0x74, 0x11, 0x41, 0x8a,
        0x01, 0x41, 0x88, 0x00, 0x49, 0xff, 0xc0, 0x49, 0xff, 0xc1, 0x49, 0xff, 0xca, 0xeb, 0xea,
        0x31, 0xc0, 0xc3, 0xb8, 0x01, 0x00, 0x00, 0x00, 0xc3,
    ]);
}

fn emit_array_remove_helper(code: &mut Vec<u8>) {
    code.extend_from_slice(&[0x48, 0x85, 0xc9]);
    let failed = emit_near_jump_placeholder(code, 0x84);
    code.extend_from_slice(&[0x48, 0x8b, 0x41, 0x08, 0x48, 0x39, 0xc2]);
    let invalid = emit_near_jump_placeholder(code, 0x83);
    code.extend_from_slice(&[
        ...
        0x89, 0xd2, 0x4d, 0x0f, 0xaf, 0xd0, 0x4d, 0x01, 0xca, 0x4d, 0x89, 0xd3, 0x4d, 0x01, 0xc3,
        0x48, 0x29, 0xd0, 0x49, 0x0f, 0xaf, 0xc0, 0x4c, 0x89, 0xd7, 0x4c, 0x89, 0xde, 0x48, 0x89,
...
fn emit_array_insert_helper(code: &mut Vec<u8>) {
    code.extend_from_slice(&[0x48, 0x85, 0xc9]);
    let failed = emit_near_jump_placeholder(code, 0x84);
    code.extend_from_slice(&[0x48, 0x8b, 0x41, 0x08, 0x48, 0x39, 0xc2]);
    let invalid_index = emit_near_jump_placeholder(code, 0x87);
    code.extend_from_slice(&[0x48, 0x3b, 0x41, 0x10]);
    let full = emit_near_jump_placeholder(code, 0x83);
    code.extend_from_slice(&[
        0x4c, 0x8b, 0x49, 0x18, 0x4c, 0x8b, 0x11, 0x49, 0x89, 0xd3, 0x4d, 0x0f, 0xaf, 0xd9, 0x4d,
        0x01, 0xd3, 0x48, 0xff, 0x41, 0x08, 0x48, 0x29, 0xd0, 0x49, 0x0f, 0xaf, 0xc1, 0x48, 0x85,
        0xc0,
    ]);
    let no_shift = emit_near_jump_placeholder(code, 0x84);
    code.extend_from_slice(&[
        0x4c, 0x89, 0xde, 0x48, 0x01, 0xc6, 0x48, 0x89, 0xf7, 0x4c, 0x01, 0xcf, 0x48, 0xff, 0xce,
        0x48, 0xff, 0xcf, 0x48, 0x89, 0xc1, 0xfd, 0xf3, 0xa4, 0xfc,
    ]);
    let copy_value = code.len();
    code.extend_from_slice(&[
        0x4c, 0x89, 0xc6, 0x4c, 0x89, 0xdf, 0x4c, 0x89, 0xc9, 0xf3, 0xa4, 0x31, 0xc0, 0xc3,
    ]);
    let failure = code.len();
    code.extend_from_slice(&[0xb8, 0x01, 0x00, 0x00, 0x00, 0xc3]);
    patch_near_jump(code, failed, failure);
...
fn emit_array_extend_helper(code: &mut Vec<u8>) {
    code.extend_from_slice(&[
        0x48, 0x85, 0xc9, 0x74, 0x4b, 0x48, 0x85, 0xd2, 0x74, 0x46, 0x4c, 0x8b, 0x49, 0x18, 0x4c,
        0x3b, 0x4a, 0x18, 0x75, 0x3c, 0x4c, 0x8b, 0x41, 0x08, 0x4c, 0x8b, 0x52, 0x08, 0x4d, 0x89,
        0xc3, 0x4d, 0x01, 0xd3, 0x4c, 0x3b, 0x59, 0x10, 0x77, 0x28, 0x4c, 0x01, 0x51, 0x08, 0x4d,
        0x85, 0xd2, 0x74, 0x1c, 0x4c, 0x8b, 0x19, 0x4d, 0x0f, 0xaf, 0xc1, 0x4d, 0x01, 0xc3, 0x4c,
        0x8b, 0x02, 0x4c, 0x89, 0xdf, 0x4c, 0x89, 0xc6, 0x4c, 0x89, 0xd1, 0x49, 0x0f, 0xaf, 0xc9,
        0xf3, 0xa4, 0x31, 0xc0, 0xc3, 0xb8, 0x01, 0x00, 0x00, 0x00, 0xc3,
    ]);
}

fn emit_array_copy_helper(code: &mut Vec<u8>) {
    code.extend_from_slice(&[
        0x48, 0x85, 0xc9, 0x74, 0x5c, 0x4d, 0x85, 0xc0, 0x74, 0x57, 0x4c, 0x8b, 0x54, 0x24, 0x28,
        0x4c, 0x8b, 0x59, 0x18, 0x4d, 0x3b, 0x58, 0x18, 0x75, 0x48, 0x48, 0x8b, 0x41, 0x08, 0x48,
        0x39, 0xc2, 0x77, 0x3f, 0x48, 0x29, 0xd0, 0x49, 0x39, 0xc2, 0x77, 0x37, 0x49, 0x8b, 0x40,
        0x08, 0x49, 0x39, 0xc1, 0x77, 0x2e, 0x4c, 0x29, 0xc8, 0x49, 0x39, 0xc2, 0x77, 0x26, 0x48,
        0x89, 0xd0, 0x49, 0x0f, 0xaf, 0xc3, 0x48, 0x8b, 0x39, 0x48, 0x01, 0xc7, 0x4c, 0x89, 0xc8,
        0x49, 0x0f, 0xaf, 0xc3, 0x49, 0x8b, 0x30, 0x48, 0x01, 0xc6, 0x4c, 0x89, 0xd1, 0x49, 0x0f,
        0xaf, 0xcb, 0xf3, 0xa4, 0x31, 0xc0, 0xc3, 0xb8, 0x01, 0x00, 0x00, 0x00, 0xc3,
    ]);
}
...
fn emit_array_resize_helper(code: &mut Vec<u8>) {
    ...
    code.extend_from_slice(&[0x48, 0x89, 0x51, 0x08, 0x48, 0x29, 0xc2, 0x48, 0x85, 0xd2]);
    let no_fill = emit_short_jump_placeholder(code, 0x74);
    code.extend_from_slice(&[
        0x48, 0x83, 0xec, 0x38, 0x4c, 0x89, 0x44, 0x24, 0x20, 0x48, 0x89, 0x54, 0x24, 0x28, 0x4c,
        0x8b, 0x49, 0x18, 0x4c, 0x89, 0x4c, 0x24, 0x30, 0x49, 0x0f, 0xaf, 0xc1, 0x48, 0x8b, 0x39,
        0x48, 0x01, 0xc7, 0x48, 0x8b, 0x44, 0x24, 0x28,
    ]);
    let fill_loop = code.len();
    code.extend_from_slice(&[0x48, 0x85, 0xc0]);
    let fill_done = emit_short_jump_placeholder(code, 0x74);
    code.extend_from_slice(&[
        0x48, 0x8b, 0x4c, 0x24, 0x30, 0x48, 0x8b, 0x74, 0x24, 0x20, 0xf3, 0xa4, 0x48, 0xff, 0xc8,
    ]);
...
fn emit_path_parent_helper(code: &mut Vec<u8>, layout: &Layout, slice_target: u32) {
    code.extend_from_slice(&[
        0x48, 0x83, 0xec, 0x78, 0x48, 0x89, 0x4c, 0x24, 0x40, 0x45, 0x31, 0xd2, 0x45, 0x31, 0xdb,
    ]);
    let loop_start = code.len();
    ...
    emit_short_jump_back(code, advance);
    let done_target = code.len();
    code.extend_from_slice(&[0x48, 0x8b, 0x4c, 0x24, 0x40, 0x31, 0xd2, 0x4d, 0x89, 0xd8]);
    emit_direct_call(code, layout.text_rva, slice_target);
    code.extend_from_slice(&[0x48, 0x83, 0xc4, 0x78, 0xc3]);
    patch_short_jump(code, done, done_target);
    patch_short_jump(code, slash, separator);
...
fn emit_path_stem_helper(code: &mut Vec<u8>, layout: &Layout, slice_target: u32) {
    code.extend_from_slice(&[
        0x48, 0x83, 0xec, 0x78, 0x48, 0x89, 0x4c, 0x24, 0x40, 0x45, 0x31, 0xd2, 0x45, 0x31, 0xdb,
        0x45, 0x31, 0xc9,
    ]);
    ...
    let dot_at_start = emit_short_jump_placeholder(code, 0x74);
    code.extend_from_slice(&[0x4d, 0x89, 0xd1]);
    let advance = code.len();
    code.extend_from_slice(&[0x49, 0xff, 0xc2]);
    emit_short_jump_back(code, loop_start);
    let separator = code.len();
    code.extend_from_slice(&[0x4d, 0x8d, 0x5a, 0x01, 0x45, 0x31, 0xc9]);
    emit_short_jump_back(code, advance);
    let done_target = code.len();
    code.extend_from_slice(&[0x4d, 0x85, 0xc9]);
    ...
    let slice_args = code.len();
    code.extend_from_slice(&[
        0x48, 0x8b, 0x4c, 0x24, 0x40, 0x4c, 0x89, 0xda, 0x4d, 0x89, 0xc8,
    ]);
    emit_direct_call(code, layout.text_rva, slice_target);
    code.extend_from_slice(&[0x48, 0x83, 0xc4, 0x78, 0xc3]);
    patch_short_jump(code, done, done_target);
...
fn emit_path_without_extension_helper(code: &mut Vec<u8>, layout: &Layout, slice_target: u32) {
    code.extend_from_slice(&[
        0x48, 0x83, 0xec, 0x78, 0x48, 0x89, 0x4c, 0x24, 0x40, 0x45, 0x31, 0xd2, 0x45, 0x31, 0xdb,
        0x45, 0x31, 0xc9,
    ]);
    ...
    code.extend_from_slice(&[0x4d, 0x89, 0xd1]);
    let slice_args = code.len();
    code.extend_from_slice(&[0x48, 0x8b, 0x4c, 0x24, 0x40, 0x31, 0xd2, 0x4d, 0x89, 0xc8]);
    emit_direct_call(code, layout.text_rva, slice_target);
    code.extend_from_slice(&[0x48, 0x83, 0xc4, 0x78, 0xc3]);
    patch_short_jump(code, done, done_target);
    patch_short_jump(code, slash, separator);
...
fn emit_string_from_byte_helper(code: &mut Vec<u8>, layout: &Layout) {
    ...
    code.extend_from_slice(&[0x31, 0xc9]);
    code.extend_from_slice(&[0xba, 0x0a, 0x00, 0x00, 0x00, 0x48, 0x89, 0x54, 0x24, 0x18]);
    code.extend_from_slice(&[0x41, 0xb8, 0x00, 0x30, 0x00, 0x00]);
    code.extend_from_slice(&[0x41, 0xb9, 0x04, 0x00, 0x00, 0x00]);
    emit_call_iat(code, layout, layout.virtual_alloc_iat);
    code.extend_from_slice(&[0x48, 0x85, 0xc0]);
    let failed = emit_short_jump_placeholder(code, 0x74);
    code.extend_from_slice(&[
        0x48, 0x8b, 0x54, 0x24, 0x18, 0x48, 0x89, 0x10, 0x48, 0x83, 0xc0, 0x08, 0x8a, 0x54, 0x24,
        0x20,
    ]);
...
fn emit_string_clone_helper(code: &mut Vec<u8>, layout: &Layout) {
    ...
    let allocation_failed = emit_short_jump_placeholder(code, 0x74);
    code.extend_from_slice(&[
        0x48, 0x89, 0x44, 0x24, 0x48, 0x48, 0x8b, 0x44, 0x24, 0x48, 0x48, 0x8b, 0x54, 0x24, 0x30,
        0x48, 0x89, 0x10,
    ]);
...
fn emit_string_slice_helper(code: &mut Vec<u8>, layout: &Layout) {
    ...
    code.extend_from_slice(&[0x48, 0x83, 0xc2, 0x08, 0x48, 0x89, 0x54, 0x24, 0x70]);
    code.extend_from_slice(&[0x31, 0xc9, 0x41, 0xb8, 0x00, 0x30, 0x00, 0x00]);
    code.extend_from_slice(&[0x41, 0xb9, 0x04, 0x00, 0x00, 0x00]);
    emit_call_iat(code, layout, layout.virtual_alloc_iat);
    code.extend_from_slice(&[0x48, 0x85, 0xc0]);
    let allocation_failed = emit_near_conditional_placeholder(code, 0x84);
    code.extend_from_slice(&[0x48, 0x8b, 0x54, 0x24, 0x70, 0x48, 0x89, 0x10]);
    ...
    code.extend_from_slice(&[0x4c, 0x8b, 0x5c, 0x24, 0x68]);
    ...
    let zero_target = code.len();
    code.extend_from_slice(&[0x45, 0x31, 0xc0, 0x4c, 0x89, 0x44, 0x24, 0x60]);
    let zero_to_allocation = emit_near_unconditional_placeholder_pe(code);
...
fn emit_alloc_copy_helper(code: &mut Vec<u8>, layout: &Layout) {
    ...
    code.extend_from_slice(&[0x31, 0xc9]);
    code.extend_from_slice(&[0x48, 0x8b, 0x54, 0x24, 0x40]);
    code.extend_from_slice(&[0x48, 0x83, 0xc2, 0x08, 0x48, 0x89, 0x54, 0x24, 0x28]);
    code.extend_from_slice(&[0x41, 0xb8, 0x00, 0x30, 0x00, 0x00]);
    code.extend_from_slice(&[0x41, 0xb9, 0x04, 0x00, 0x00, 0x00]);
    emit_call_iat(code, layout, layout.virtual_alloc_iat);
    code.extend_from_slice(&[0x48, 0x85, 0xc0]);
    let failed = emit_short_jump_placeholder(code, 0x74);
    code.extend_from_slice(&[
        0x48, 0x8b, 0x54, 0x24, 0x28, 0x48, 0x89, 0x10, 0x48, 0x83, 0xc0, 0x08, 0x48, 0x89, 0x44,
        0x24, 0x20,
    ]);
...
fn emit_read_file_helper(code: &mut Vec<u8>, layout: &Layout) {
    ...
    code.extend_from_slice(&[0x48, 0x85, 0xc0]);
    let allocation_failed = emit_near_jump_placeholder(code, 0x84);
    code.extend_from_slice(&[0x48, 0x89, 0x10]);
    code.extend_from_slice(&[0x48, 0x83, 0xc0, 0x08]);
    code.extend_from_slice(&[0x48, 0x89, 0x44, 0x24, 0x48]);
...
fn emit_truncate_file_helper(code: &mut Vec<u8>, layout: &Layout) {
    // Win64 ABI: keep the path, requested size, and handle outside call shadow space.
    code.extend_from_slice(&[0x48, 0x83, 0xec, 0x58]);
    code.extend_from_slice(&[0x48, 0x89, 0x4c, 0x24, 0x40]);
    code.extend_from_slice(&[0x48, 0x89, 0x54, 0x24, 0x48]);
    code.extend_from_slice(&[0x48, 0x8b, 0x4c, 0x24, 0x40]);
    ...
    code.extend_from_slice(&[0x48, 0x89, 0x44, 0x24, 0x50]);
    code.extend_from_slice(&[0x48, 0x83, 0xf8, 0xff]);
    let invalid_handle = emit_near_conditional_placeholder(code, 0x84);
    ...
    code.extend_from_slice(&[0x85, 0xc0]);
    let pointer_failed = emit_near_conditional_placeholder(code, 0x84);

    code.extend_from_slice(&[0x48, 0x8b, 0x4c, 0x24, 0x50]);
    emit_call_iat(code, layout, layout.set_end_of_file_iat);
    code.extend_from_slice(&[0x85, 0xc0]);
    let end_failed = emit_near_conditional_placeholder(code, 0x84);
...
fn emit_file_read_to_string_helper(code: &mut Vec<u8>, layout: &Layout) {
    ...
    code.extend_from_slice(&[0x48, 0x85, 0xc0]);
    let allocation_failed = emit_near_jump_placeholder(code, 0x84);
    code.extend_from_slice(&[0x48, 0x8b, 0x54, 0x24, 0x50, 0x48, 0x89, 0x10]);
    code.extend_from_slice(&[0x48, 0x83, 0xc0, 0x08, 0x48, 0x89, 0x44, 0x24, 0x40]);
...
fn build_idata(layout: &Layout) -> Vec<u8> {
    ...
    } else if layout.has_file_ops || layout.has_file_truncate {
        imports.push(b"CreateFileA\0".as_slice());
        imports.push(b"CloseHandle\0".as_slice());
    }
...
fn build_image(layout: &Layout, text: &[u8], rdata: &[u8], idata: &[u8]) -> Vec<u8> {
    ...
    write_u32(
        &mut out,
        0xd0,
        align_to(layout.idata_rva + idata.len() as u32, SECTION_ALIGNMENT),
    );
