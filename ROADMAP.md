...
Progress: `compiler/geo_syntax` now owns the AST, token model, lexer, and parser, `compiler/geo_ir` owns the typed machine-independent IR, `compiler/geo_semantic` owns resolution, type checking, borrow checking, and runtime symbol metadata, and `compiler/geo_codegen` owns AST-to-IR lowering. The CLI crate keeps compatibility re-exports while machine backends are split.
