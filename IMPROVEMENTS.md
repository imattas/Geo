...
- Add native runtime coverage for recursive directory mutation and the remaining directory APIs. Basic `create_dir`/`remove_dir` mutation, `rename_file`, `copy_file`, file timestamp queries, `dir_entry_count`, `dir_entry_name`, `dir_entry_path`, and `process_id` now have direct two-target coverage.
