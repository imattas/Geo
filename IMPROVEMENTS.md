...
- Add native runtime coverage for directory entry paths, recursive directory mutation, and the remaining directory APIs. Basic `create_dir`/`remove_dir` mutation, `rename_file`, `copy_file`, file timestamp queries, `dir_entry_count`, and `dir_entry_name` now have direct two-target coverage.
