...
- Add native runtime coverage for directory entry names/paths, recursive directory mutation, and the remaining directory APIs. Basic `create_dir`/`remove_dir` mutation, `rename_file`, `copy_file`, file timestamp queries, and `dir_entry_count` now have direct two-target coverage.
