...
results, and handle-based `file_read_to_string` results now store the
compiler-owned mapping header and can be released through
`std.string.string_free`; apply the same contract next to line-read results.
