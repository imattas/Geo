...
- direct PE64 executable emission now wraps compiled Win64 machine code for `main`, internal calls, `.rdata` references, bounds-check calls, `string_len`, `string_byte_at`, `string_concat`, and simple `print`/`println` string console output before calling `ExitProcess`
